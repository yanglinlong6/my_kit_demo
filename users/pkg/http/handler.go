...
func makeByeHandler(m *http.ServeMux, endpoints endpoint.Endpoints, options []http1.ServerOption) {
	m.Handle("/bye", http1.NewServer(endpoints.ByeEndpoint, decodeByeRequest, encodeByeResponse, options...))
}

// decodeByeRequest is a transport/http.DecodeRequestFunc that decodes a
// JSON-encoded request from the HTTP request body.
func decodeByeRequest(_ context.Context, r *http.Request) (interface{}, error) {
	req := endpoint.ByeRequest{}
	err := json.NewDecoder(r.Body).Decode(&req)
	return req, err
}

// encodeByeResponse is a transport/http.EncodeResponseFunc that encodes
// the response as JSON to the response writer
func encodeByeResponse(ctx context.Context, w http.ResponseWriter, response interface{}) (err error) {
	if f, ok := response.(endpoint.Failure); ok && f.Failed() != nil {
		ErrorEncoder(ctx, f.Failed(), w)
		return nil
	}
